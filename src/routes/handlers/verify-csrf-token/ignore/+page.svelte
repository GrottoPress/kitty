<script lang="ts">
  ...
  const jsonWithToken = async (event: Event) => {
    ...
    response = await data.fetch(`${endpoint}.json/ignore`, {
      method: 'POST',
      headers,
...
<form id="with-token" method="POST" action="{endpoint}/ignore?/create">
  <input type="hidden" name={data.csrfParamKey} value={data.csrfToken} />
  <input type="hidden" name="city" value="Kumasi" />
  <button type="submit">Send</button>
</form>

<form id="without-token" method="POST" action="{endpoint}/ignore?/create">
  <input type="hidden" name="city" value="Kumasi" />
  <button type="submit">Send</button>
</form>
