...
### Fixed
- Remove wrapper markup around `Connection` children
...
### Added
- Add global `Connection.connection` state to track connection status

### Fixed
- Fix potential race condition with `.clearTimeout()` in `Connection` component

### Changed
- Replace all snippet props with `children` snippet in `Connection` component
...
### Added
- Add `ToggleButton.keepOpen` prop that keeps the target open upon navigation

## [0.14.2] - 2025-04-27

### Fixed
- Close `ToggleButton` before navigation (not after)
